// Copyright (c) 2017-2019 The ROCKETFUNDCOIN developers
                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zRKT ought to be enough for anybody." - Bill Gates, 2017
    ui->zRKTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRKTSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRKTpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzRKT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRKT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zRKT, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRKT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRKT in ") +
...
void PrivacyDialog::on_pushButtonSpendzRKT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRKT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zRKT, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zRKT
        sendzRKT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRKT
    sendzRKT();
}

void PrivacyDialog::on_pushButtonZRktControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZRktControlDialog* zRktControl = new ZRktControlDialog(this);
    zRktControl->setModel(walletModel);
    zRktControl->exec();
}

void PrivacyDialog::setZRktControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRktSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRKT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid RocketFundCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zRKTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRKT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " RKT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRKT</b>";
    ...
    // use mints from zRKT selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZRktControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZRktControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zRKT transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zRKT was spent successfully update the addressbook with the label
    ...
    // Clear zrkt selector in case it was used
    ZRktControlDialog::setSelectedMints.clear();
    ui->labelzRktSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRKT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Rkt, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zRKT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zrktTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRKT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRKT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRKT "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zRKT "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zRKT </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zRKT </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zRKT </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzRKTSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzRKT->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zRKT
        //ui->pushButtonMintzRKT->setEnabled(false);
        //ui->pushButtonMintzRKT->setToolTip(tr("zRKT is currently disabled due to maintenance."));

        // Spend zRKT
        ui->pushButtonSpendzRKT->setEnabled(false);
        ui->pushButtonSpendzRKT->setToolTip(tr("zRKT is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zRKT
        //ui->pushButtonMintzRKT->setEnabled(true);
        //ui->pushButtonMintzRKT->setToolTip(tr("PrivacyDialog", "Enter an amount of RKT to convert to zRKT", 0));

        // Spend zRKT
        ui->pushButtonSpendzRKT->setEnabled(true);
        ui->pushButtonSpendzRKT->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
