...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZRKTModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zrktTracker->HasMintTx(hash))) {
            //zRKT stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZRKT;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zRkt;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by ROCKETFUNDCOIN Address
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            ...
            if (mapValue["DS"] == "1") {
                ...
                if (ExtractDestination(wtx.vout[0].scriptPubKey, address)) {
                    // Sent to ROCKETFUNDCOIN Address
                    sub.address = CBitcoinAddress(address).ToString();
                } else {
...
bool IsZRKTType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZRKT:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zRkt:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZRKT || type == TransactionRecord::MNReward) {
