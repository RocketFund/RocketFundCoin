...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::RKT);
    ...
    if (!settings.contains("fZeromintEnable"))
        settings.setValue("fZeromintEnable", Params().ZeroCoinEnabled());
    fEnableZeromint = settings.value("fZeromintEnable").toBool();

    if (!settings.contains("fEnableAutoConvert"))
        settings.setValue("fEnableAutoConvert", Params().ZeroCoinEnabled());
...
// Wallet
    ...
    if (settings.contains("nAnonymizeRocketFundCoinAmount"))
        SoftSetArg("-anonymizerocketfundcoinamount", settings.value("nAnonymizeRocketFundCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in rocketfundcoin.cpp during shotdown to also remove the window positions
