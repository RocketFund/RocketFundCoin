...
#include "ui_zrktcontroldialog.h"
...
std::set<std::string> ZRktControlDialog::setSelectedMints;
std::set<CMintMeta> ZRktControlDialog::setMints;

bool CZRktControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRktControlDialog::COLUMN_DENOMINATION || column == ZRktControlDialog::COLUMN_VERSION || column == ZRktControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRktControlDialog::ZRktControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRktControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRktControlDialog::~ZRktControlDialog()
{
    delete ui;
}

void ZRktControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRktControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRktControlWidgetItem* itemDenom(new CZRktControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRktControlWidgetItem *itemMint = new CZRktControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zrktTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zrktTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRKT.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRKT seed used to mint this zRKT is not the same as currently hold in the wallet";
            else
...
void ZRktControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRktControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRkt_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRktControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRktControlDialog::GetSelectedMints()
{
...
void ZRktControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
