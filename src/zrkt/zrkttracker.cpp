// Copyright (c) 2018-2019 The ROCKETFUNDCOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zrkt/deterministicmint.h>
#include "zrkttracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzRKTTracker::CzRKTTracker(std::string strWalletFile)
{
...
CzRKTTracker::~CzRKTTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzRKTTracker::Init()
{
...
bool CzRKTTracker::Archive(CMintMeta& meta)
{
...
bool CzRKTTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzRKTTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzRKTTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzRKTTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzRKTTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzRKTTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzRKTTracker::GetSerialHashes()
{
...
CAmount CzRKTTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzRKTTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzRKTTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzRKTTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzRKTTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzRKTTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzRKTTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzRKTTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzRKTTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzRKTTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzRKTTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzRKTWallet* zRKTWallet)
{
    bool iszRKTWalletInitialized = (NULL != zRKTWallet);
    CMintMeta meta;
    ...
    if (! iszRKTWalletInitialized)
        zRKTWallet = new CzRKTWallet(strWalletFile);
    meta.isSeedCorrect = zRKTWallet->CheckSeed(dMint);
    if (! iszRKTWalletInitialized)
        delete zRKTWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzRKTTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzRKTTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzRKTTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzRKTTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzRKTTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzRKTTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzRKTWallet* zRKTWallet = new CzRKTWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zRKTWallet);
        }
        delete zRKTWallet;
        LogPrint("zero", "%s: added %d dzrkt from DB\n", __func__, listDeterministicDB.size());
    }
