...
CzRKTWallet::CzRKTWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zrkt seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZRKTSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZRKTSeed_deprecated()) {
                    LogPrintf("%s: Updated zRKT seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zrkt seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zrkt wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzRKTWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZRKTCount(nCountLastUsed);
    else if (!walletdb.ReadZRKTCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzRKTWallet::Lock()
{
    seedMaster = 0;
}

void CzRKTWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzRKTWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZRKT(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzRKTWallet::LoadMintPoolFromDB()
{
...
void CzRKTWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzRKTWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzRKTWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zrktTracker->HasPubcoinHash(pMint.first)) {
...
bool CzRKTWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZRKT(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zrktTracker which also adds to database
    pwalletMain->zrktTracker->Add(dMint, true);
...
void CzRKTWallet::SeedToZRKT(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzRKTWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzRKTWallet::GenerateDeterministicZRKT(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzRKTWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzRKTWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzRKTWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
