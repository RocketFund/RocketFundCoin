...
public:
    ...
    bool WriteZRKTSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZRKTSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZRKTSeed_deprecated(uint256& seed);
    bool EraseZRKTSeed();
    bool EraseZRKTSeed_deprecated();

    bool WriteZRKTCount(const uint32_t& nCount);
    bool ReadZRKTCount(uint32_t& nCount);
