...
#include "zrkt/zrktwallet.h"
#include "zrkt/zrkttracker.h"
...
/**
 * Fees smaller than this (in urkt) are considered zero fee (for transaction creation)
...
{
    return zrktTracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZRKT = GetBoolArg("-zapwallettxes", false);
    if (fCheckZRKT)
        zrktTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zrkt
            if (fCheckZRKT && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zRKT in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zrktTracker->GetMints(true);
    ...
    return zrktTracker->GetBalance(false, false);
}
...
{
    return zrktTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zrktTracker->ListMints(true, true, true);
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-eskstake", true) && !fPrecompute) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CRktStake> input(new CRktStake());
            input->SetInput((CTransaction) *out.tx, out.i);
            listInputs.emplace_back(std::move(input));
        }
    }

    /* Disable zRKT Staking
    //zRKT
    if ((GetBoolArg("-zrktstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zRKT set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = zrktTracker->ListMints(true, true, fUpdate);
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZRktStake> input(new CZRktStake(meta.denom, meta.hashStake));
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZeskBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 RKT collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 RKT collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 RKT.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 RKT.");
                    } else {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-rocketfundcoin-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZRKT());
            ...
            //Mark mints as spent
            if (stakeInput->IsZRKT()) {
                CZRktStake* z = (CZRktStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zRKT. Current percentage of zRKT: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zRKT are integers > 0, so we can't mint 10% of 9 RKT
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zRKT\n", nBalance);
        return;
    }

    // Percentage of zRKT we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zRKT (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zRKT amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zRKT amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zRKT We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough RKT to mint this denomination
...
bool CWallet::CreateZRKTOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZRKT(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZRKTOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zrkt output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Rkt
    ...
    // Sign if these are rocketfundcoin outputs - NOTE that zRKT outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZRKT_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zRKT
        receipt.SetStatus(_("The coin spend has been used"), ZRKT_SPENT_USED_ZRKT);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zrktTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zrktTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zrktTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZRKT_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zrktTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zrktTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zrktTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZRKT_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zRKT privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZRKT_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZRKT_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZRKT_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZRKT_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZRKT_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZRKT_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZRKT_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZRKTModule::createInput(in, mint, hashTxOut)){
                receipt.SetStatus(_("Cannot create public spend input"), ZRKT_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZRKT_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zRKT"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZRKT_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zRKT public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zRKT used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zrktTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zRKT mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            //add output to rocketfundcoin address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZRKT_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZRKT_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zrktTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zrktTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zrktTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zrktTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzrktAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzrktAuto ? "-autozrktbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZRktBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozrktbackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozrktbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozrktbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zrktbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zrktbackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, std::vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo, bool isPublicSpend)
{
    ...
    int nStatus = ZRKT_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZRKT_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZRKT_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zrktTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZRKT_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZRKT_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zrktTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zrktTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZRKT_SPEND_OKAY);  // When we reach this point spending zRKT was successful
...
{
    CMintMeta meta;
    if (!zrktTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zrktTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zrktTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zrktTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zrktTracker->UpdateState(meta);
    } else {
...
{
    // We don't even need to worry about this code.. no zRKT.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("rocketfundcoin-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zrktTracker->GetSpendCache(serialHash);
