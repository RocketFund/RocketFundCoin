...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "rocketfundcoin" is a composite category enabling all ROCKETFUNDCOIN-related debug output
            if (ptrCategory->count(std::string("rocketfundcoin"))) {
...
#else
    const char* pszModule = "rocketfundcoin";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ROCKETFUNDCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ROCKETFUNDCOIN
// Mac: ~/Library/Application Support/ROCKETFUNDCOIN
// Unix: ~/.rocketfundcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ROCKETFUNDCOIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "rocketfundcoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty rocketfundcoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override rocketfundcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "rocketfundcoind.pid"));
