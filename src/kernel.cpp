...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s :{ nStakeModifier=%s\n"
                            ...
                            "}\n",
            __func__, HexStr(modifier_ss), ((stake->IsZRKT()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())));
    }
    return true;
}
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zRKT stake block is less than allowed start height", __func__);

    if (CZRktStake* zRKT = dynamic_cast<CZRktStake*>(stake)) {
        CBlockIndex* pindexFrom = zRKT->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zRKT stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zRKT stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zRKT->GetValue()));
        if (nChecksum200 != zRKT->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zRKT->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZRktStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zRKT fails context checks", __func__);
    } else {
        ...
        CRktStake* eskInput = new CRktStake();
        eskInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(eskInput);
    }
    return true;
}
