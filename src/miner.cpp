...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (hasZerocoinSpends) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zrkts that have been in mempool long
                    //and higher priority to zrkts that are large in value
                    ...
                    // zRKT spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zRKT spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zRKT serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (fZerocoinActive) {
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                //For the period before v2 activation, zRKT will be disabled and previous block's checkpoint is all that will be needed
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zRKT with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the zRKT key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("%s: Signing new block with zRKT key failed \n", __func__);
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ROCKETFUNDCOINMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            pwalletMain->zrktTracker->RemovePending(pblock->vtx[1].GetHash());
            pwalletMain->zrktTracker->ListMints(true, true, true); //update the state
        }
        return error("ROCKETFUNDCOINMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ROCKETFUNDCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rocketfundcoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ROCKETFUNDCOINMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ROCKETFUNDCOINMiner exception");
    } catch (...) {
        LogPrintf("ROCKETFUNDCOINMiner exception");
    }

    LogPrintf("ROCKETFUNDCOINMiner exiting\n");
}
