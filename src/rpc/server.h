...
/**
 * ROCKETFUNDCOIN RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZeskSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzrktseed(const UniValue& params, bool fHelp);
extern UniValue getzrktseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzrkt(const UniValue& params, bool fHelp);
extern UniValue dzrktstate(const UniValue& params, bool fHelp);
